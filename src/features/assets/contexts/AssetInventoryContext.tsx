...
interface AssetInventoryContextType extends Omit<AssetInventoryState, 'showImportModal'> {
  stats: AssetStats;
  ...
  isImportModalOpen: boolean; // Expose the boolean state value
...
export const AssetInventoryProvider: React.FC<AssetInventoryProviderProps> = ({ children }) => {
  ...
  // Extract showImportModal boolean before it gets overwritten by the function
  const { showImportModal: showImportModalBool, ...stateWithoutShowImportModal } = state;
  
  const value: AssetInventoryContextType = {
    ...stateWithoutShowImportModal,
    stats,
    paginatedAssets,
    filterOptions,
    totalPages,
    ...
    showImportModal,
    hideImportModal,
    isImportModalOpen: showImportModalBool, // Expose the boolean value
    deleteAssets,
    addAsset,
    updateAsset,
    replaceAssets,
