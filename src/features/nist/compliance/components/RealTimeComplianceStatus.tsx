...
const RefreshCw = LucideIconsAny.RefreshCw as React.ComponentType<{ className?: string; [key: string]: unknown }>;
...
const { Shield, BarChart3, FileText, Target } = LucideIcons;
...
export const RealTimeComplianceStatus: React.FC<RealTimeComplianceStatusProps> = ({
  ...
  const functionComplianceRefs = useRef<Map<string, HTMLDivElement>>(new Map());
  const controlImplementationRef = useRef<HTMLDivElement>(null);
  const evidenceCollectionRef = useRef<HTMLDivElement>(null);
  ...
  useEffect(() => {
    if (complianceData) {
      if (overallComplianceRef.current) {
        overallComplianceRef.current.style.width = `${complianceData.overallCompliance}%`;
      }
      
      Object.entries(complianceData.functionCompliance).forEach(([functionName, score]) => {
        const ref = functionComplianceRefs.current.get(functionName);
        if (ref) {
          ref.style.width = `${score}%`;
        }
      });
      
      if (controlImplementationRef.current) {
        controlImplementationRef.current.style.width = `${complianceData.controlImplementationProgress}%`;
      }
      
      if (evidenceCollectionRef.current) {
        evidenceCollectionRef.current.style.width = `${complianceData.evidenceCollectionProgress}%`;
      }
    }
  }, [complianceData]);

  const setFunctionComplianceRef = (functionName: string, score: number) => (element: HTMLDivElement | null) => {
    if (element) {
      functionComplianceRefs.current.set(functionName, element);
      element.style.width = `${score}%`;
    } else {
      functionComplianceRefs.current.delete(functionName);
    }
  };
          ...
          {Object.entries(complianceData.functionCompliance).map(([func, score]) => (
            ...
            >
              ...
              <div className="w-full bg-gray-200 dark:bg-gray-700 rounded-full h-2 mb-2">
                <div
                  ref={setFunctionComplianceRef(func, score)}
