...
export const useAssetRelationships = (assets: Asset[], relationships: AssetRelationship[]) => {
  ...
  const getCriticalityImpactChain = useCallback((assetId: string, visited = new Set<string>()): Asset[] => {
  ...
  }, [assets, dependencyMap]);

  const getAssetDependencies = useCallback((assetId: string, depth = 3): Asset[] => {
  ...
  const calculateCriticalityScore = useCallback((assetId: string): number => {
    const asset = assets.find(a => a.id === assetId);
    if (!asset) return 0;
  ...
  }, [assets, getCriticalityImpactChain]);
