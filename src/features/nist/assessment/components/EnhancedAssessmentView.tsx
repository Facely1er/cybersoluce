...
const { FileText, Clock, BarChart3, Users, Shield } = LucideIcons;
import { Breadcrumbs } from '../../../../shared/components/layout';
import { useInternalLinking } from '../../../../shared/hooks/useInternalLinking';
import { AssessmentData, Framework, Question, QuestionEvidence, EvidenceItem } from '../../../../shared/types';
...
export const EnhancedAssessmentView: React.FC<EnhancedAssessmentViewProps> = ({
  ...
  const [availableTeamMembers] = useState<Array<{
    id: string;
    name: string;
    email: string;
    role: string;
  ...
  // Calculate progress
  const totalQuestions = framework.sections.reduce((sum: number, section: { categories: Array<{ questions: unknown[] }> }) => 
  ...
  const handleAddEvidence = useCallback((questionId: string, evidence: QuestionEvidence) => {
  ...
  const handleUploadEvidence = useCallback((_file: File, metadata: Record<string, unknown>) => {
    // Simulate file upload - in a real app, this would upload to a server
    const newEvidence: EvidenceItem = {
      id: Date.now().toString(),
      name: metadata.name as string || 'Untitled Evidence',
      type: (metadata.type as EvidenceItem['type']) || 'document',
      description: metadata.description as string || '',
      uploadedAt: new Date(),
      uploadedBy: metadata.uploadedBy as string || 'Current User',
      ...
      status: 'active',
      confidentialityLevel: (metadata.confidentialityLevel as EvidenceItem['confidentialityLevel']) || 'internal',
  ...
  const handleCreateTasksFromQuestion = useCallback(async (questionId: string) => {
    ...
    if (assignedUsers.length === 0) {
      alert('Please assign team members to this question first');
      return;
    }

    try {
      // Mock task service - in a real app this would be imported
      const taskService = {
        assignTasksFromAssessment: async (_assessmentId: string, _sectionId: string, questionIds: string[], assignedUsers: string[], userId: string) => {
      ...
      // This would integrate with the task service
      const newTasks = await taskService.assignTasksFromAssessment(
        assessment.id,
        currentSection.id,
        [questionId],
        assignedUsers,
      ...
      alert(`Created ${newTasks.length} task(s) for this question`);
    } catch (error) {
      console.error('Failed to create tasks:', error);
      alert('Failed to create tasks');
    }
  }, [currentQuestion, questionAssignments, assessment, currentSection]);
        ...
        <div className="lg:col-span-1">
          <div className="bg-white/90 dark:bg-gray-800/90 backdrop-blur-sm rounded-xl shadow-xl border border-gray-200 dark:border-gray-700 sticky top-24">
            ...
            <div className="p-6 bg-gradient-to-r from-gray-50 to-blue-50 dark:from-gray-700 dark:to-blue-900/20 rounded-b-xl">
              <label htmlFor="auto-save-toggle" className="flex items-center space-x-3">
                <input
                  id="auto-save-toggle"
                  ...
                  <div className="flex-1">
                    ...
                    <div className="space-y-4 mb-6">
                      {currentQuestion.options.map((option: { value: number; label: string; description?: string; riskLevel?: string; recommendedActions?: string[] }) => (
                        ...
                        >
                          <input
                            id={`question-${currentQuestion.id}-option-${option.value}`}
                            type="radio"
                      ...
                      <div className="mb-6 p-6 bg-gradient-to-r from-purple-50 to-pink-50 dark:from-purple-900/20 dark:to-pink-900/20 rounded-xl border border-purple-200 dark:border-purple-700 shadow-lg">
                        ...
                        <div className="space-y-3">
                          {availableTeamMembers.map(member => (
                            <label htmlFor={`question-${currentQuestion.id}-member-${member.id}`} key={member.id} className="flex items-center space-x-3 p-3 bg-white dark:bg-gray-700 rounded-lg border border-purple-200 dark:border-purple-600">
                              <input
                                id={`question-${currentQuestion.id}-member-${member.id}`}
                                name={`question-${currentQuestion.id}-member-${member.id}`}
                                type="checkbox"
                    ...
                    {showNotes && (
                      <div className="mb-6 p-6 bg-gradient-to-r from-blue-50 to-indigo-50 dark:from-blue-900/20 dark:to-indigo-900/20 rounded-xl border border-blue-200 dark:border-blue-700 shadow-lg">
                        ...
                        <label htmlFor={`question-notes-${currentQuestion.id}`} className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">
                          Add your observations, context, or additional information:
                        </label>
                        <textarea
                          id={`question-notes-${currentQuestion.id}`}
                          name={`question-notes-${currentQuestion.id}`}
